...
/**
@brief Konsolowy interfejs użytkownika aplikacji „Budżet domowy”.

Klasa stanowi punkt wejścia programu (metoda @c main) i udostępnia menu do:
- zarządzania transakcjami (przychody/wydatki),
- ustawiania budżetów i limitów,
- generowania raportów,
- uruchamiania prognoz,
- eksportu danych do CSV.

@details
W tej klasie widoczne jest użycie kilku wzorców:
- Singleton: pobranie instancji @c MenedżerBudżetu (@ref wzorzec_singleton),
- Fabryka raportów: tworzenie obiektów raportów (@ref wzorzec_factory),
- Strategia prognoz: wybór algorytmu prognozy (@ref wzorzec_strategy),
- Adapter eksportu: eksport do CSV (@ref wzorzec_adapter),
- Obserwator: rejestracja powiadomień o przekroczeniu limitu (@ref wzorzec_observer).
*/
